 ...
 *
 * Sends alerts on: bet placed, trade out, system qualification, errors, startup.
...
class TelegramNotifier {
  ...
  async notifyBetPlaced({ matchName, player, side, odds, stake, sizeMatched, averagePriceMatched, sqDiffTrigger, momentumAtTrigger, system, betId, dryRun }) {
    ...
    const sqStr = (sqDiffTrigger == null)
      ? '—'
      : `${sqDiffTrigger > 0 ? '+' : ''}${Math.round(sqDiffTrigger)}`;
    const momStr = (momentumAtTrigger == null)
      ? '—'
      : `${momentumAtTrigger > 0 ? '+' : ''}${Math.round(momentumAtTrigger)}`;
    await this._send(
      `✅ *BET${dryRun ? ' \\[DRY\\]' : ''} — ${sideLabel} ${player} @ ${odds}${matchedAt}*\n` +
      `${matchName}\n` +
      `£${stake.toFixed(2)}${partialNote}  |  Mom@Trig: ${momStr}  |  ΔSQ: ${sqStr}` +
      (system ? `  |  ${system}` : '')
    );
  }
    async notifyTradeOut({ matchName, profit, reason }) {
  ...
  async _handleStatus(msg) {
    ...
    try {
      ...
      const pnlToday = this._orderManager?.getPnlToday()      ?? null;
      ...
      text =
        ...
        `P&L today: £${pnlToday != null ? (pnlToday >= 0 ? '+' : '') + pnlToday.toFixed(2) : '?'} (estimated)\n` +
  ...
  async _handleSystems(msg) {
    ...
    const notifs = Object.entries({
      ...
      'Sys match': ts.notifySystemMatch,
