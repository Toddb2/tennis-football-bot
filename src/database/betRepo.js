...
const _insert = db.prepare(`
  INSERT OR IGNORE INTO bets (
    bet_id, betfair_market_id, strategy_name,
    player_key, player_name, side,
    requested_odds, actual_odds, stake, size_matched, liability,
    placed_at, dry_run, reason, exit_config, momentum_at_bet
  ) VALUES (
    @betId, @betfairMarketId, @strategyName,
    @playerKey, @playerName, @side,
    @requestedOdds, @actualOdds, @stake, @sizeMatched, @liability,
    @placedAt, @dryRun, @reason, @exitConfig, @momentumAtBet
  )
`);
...
const _getRecent = db.prepare(`
  SELECT b.*, m.surface, m.tournament, m.match_name,
    COALESCE(
    ...
    (SELECT s.matched_volume FROM market_snapshots s
     ...
     ORDER BY s.ts DESC LIMIT 1) AS volume_at_bet
  FROM bets b
...
function insert(fields) {
  _insert.run({
    ...
    liability:       fields.liability       ?? null,
...
/** Per-strategy P&L summary for the dashboard analytics tab. */
