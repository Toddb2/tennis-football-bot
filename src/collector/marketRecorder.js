...
class MarketRecorder {
  ...
  _flushMarket(marketId) {
    ...
    // This runs synchronously from the Betfair stream's market-update handler.
    // A disk error (ENOSPC/EACCES) here would otherwise throw out of the emit and
    // reach the global uncaughtException handler → process.exit(1). Contain it:
    // dropping one market's snapshot file is preferable to crashing the bot.
    try {
      ...
      logger.error('MarketRecorder: failed to flush market snapshots', {
        marketId, count: entry.snapshots.length, message: err.message });
    }
  }
}
