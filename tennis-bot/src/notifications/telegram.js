...
class TelegramNotifier {
  ...
  async notifyBetPlaced({ matchName, player, side, odds, stake, sizeMatched, averagePriceMatched, sqDiffTrigger, system, betId, dryRun }) {
    ...
    await this._send(
      ...
      `${matchName}\n` +
      `£${stake.toFixed(2)}${partialNote}  |  ΔSQ Trigger: ${sqStr}` +
      (system ? `  |  ${system}` : '')
    );
  }
