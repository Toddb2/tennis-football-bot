 ...
 * Call before start().
 */
function init({ stateStore, orderManager, betfairClient, bfbmClient }) {
...
module.exports = { init, start, broadcast, getMatchSnapshots: buildMatchSnapshots };
...
async function apiGetUpcoming(req, res) {
  try {
    ...
    if (_betfairClient) {
      try {
        const markets = await _betfairClient.listMarketCatalogue({
          inPlayOnly:      false,
          marketTypes:     ['MATCH_ODDS'],
          marketStartTime: { from: start.toISOString(), to: end.toISOString() },
        });
  ...
  try {
    const _entryDataQuery = db.prepare(`
      SELECT
        ...
        b.placed_at, b.reason, b.dry_run,
...
function apiDbMarketScanner(req, res) {
  try {
    const rows = db.prepare(`
      ...
      LIMIT 500
    `).all();
    // Flatten the set-1 serve stats so AI/clients don't have to JSON-parse.
    for (const r of rows) {
      const raw = r.s1_serve_stats_json;
      delete r.s1_serve_stats_json;
      if (!raw) { r.s1_serve_stats = null; continue; }
      try {
        const parsed = JSON.parse(raw);
        // Prefer set1-specific stats; fall back to match-level (set 1 only ended)
        r.s1_serve_stats = parsed.set1 || parsed.match || null;
      } catch (_) {
        r.s1_serve_stats = null;
      }
    }
    res.json(rows);
  } catch (err) {
