...
class OrderManager {
  ...
  async _placeOrder(marketId, selectionId, side, odds, stake, meta = {}) {
    ...
    try {
      // Edge at bet time: signed for the bet — positive = bot thinks it has +EV.
      //   BACK → edge_for_bet_player; LAY → -edge_for_bet_player.
      const _edgeRaw = order.playerKey === 'A' ? meta.edgeA : meta.edgeB;
      const _edgeAtBet = (typeof _edgeRaw === 'number')
        ? (order.side === 'BACK' ? _edgeRaw : -_edgeRaw)
        : null;
      betRepo.insert({
        betId:           order.betId,
        betfairMarketId: order.marketId,
        strategyName:    meta.strategyName || meta.systemName || null,
        // subStrategy auto-derived in betRepo.insert from strategyName + playerKey.
        ...
        momentumAtBet:   (typeof meta.momentumIndex === 'number') ? meta.momentumIndex : null,
        edgeAtBet:       _edgeAtBet,
