...
function apiGetDebugMode(req, res) {
  res.json({
    ...
    pnlToday:     betRepo.getPnlToday().pnl,
  });
}
...
function apiGetPerformance(req, res) {
  try {
    ...
    for (const bet of placed) {
      ...
      const _stake = parseFloat(bet.stake) || null;
      const _liability = parseFloat(bet.liability)
        || (_stake != null && bet.side === 'LAY' && parseFloat(bet.odds)
              ? _stake * (parseFloat(bet.odds) - 1)
              : _stake);
    ...
    const systems = Object.values(bySystem).map(s => {
      ...
      const totalStake = settledB.reduce((sum, b) => sum + (b.stake || 0), 0);
      const winRate    = settledB.length > 0 ? wins / settledB.length * 100 : null;
      // ROI = PnL / total stake (industry standard yield on turnover).
...
function apiDbSummary(req, res) {
  try {
    ...
    res.json({
      ...
      // PnL today is sourced from the DB (settled bets where DATE(settled_at)=today),
      // not the in-memory orderManager — survives pm2 restarts.
      pnlToday:       betRepo.getPnlToday().pnl,
...
function apiDbBets(req, res) {
  try {
    ...
    for (const r of rows) {
      ...
      // Derived fields for the dashboard / CSV: sub_strategy fallback + ROI %
      if (!r.sub_strategy && r.strategy_name && r.player_key) {
        r.sub_strategy = /_P[12]$/.test(r.strategy_name)
          ? r.strategy_name
          : `${r.strategy_name}-${r.player_key === 'A' ? 'P1' : 'P2'}`;
      }
      r.roi_pct = (r.stake && r.pnl != null)
        ? +((r.pnl / r.stake) * 100).toFixed(2)
        : null;
    }

    // Enrich with surface win-rate diff (P1 wr − P2 wr on this surface) and
    // traded-volume milestones (pre-match / end of set 1 / end of set 2).
    // Surface-specific data wins; if a player has no matches on this surface,
    // we fall back to their overall WR across all surfaces (still informative
    // for cold-start players whose only data is on a different surface).
    const _surfaceWrStmt = db.prepare(`
      SELECT
      ...
      WHERE surface = @surface
        AND winner IN ('A','B')
        AND betfair_market_id != @excl
    `);
    const _overallWrStmt = db.prepare(`
      SELECT
        COALESCE(SUM(CASE WHEN (player_a_name = @name AND winner = 'A')
                            OR (player_b_name = @name AND winner = 'B') THEN 1 ELSE 0 END), 0) AS wins,
        COALESCE(SUM(CASE WHEN player_a_name = @name OR player_b_name = @name THEN 1 ELSE 0 END), 0) AS total
      ...
      WHERE winner IN ('A','B')
        AND betfair_market_id != @excl
    `);
    const _milesStmt = db.prepare(`
      SELECT milestone, matched_volume FROM price_milestones
      WHERE betfair_market_id = ? AND milestone IN ('pre_match','set_1_end','set_2_end')
    `);
    for (const r of rows) {
      // Surface WR diff (with overall-WR fallback when surface-specific data missing)
      if (r.surface && r.player_a_name && r.player_b_name) {
        const p1surf = _surfaceWrStmt.get({ name: r.player_a_name, surface: r.surface, excl: r.betfair_market_id });
        const p2surf = _surfaceWrStmt.get({ name: r.player_b_name, surface: r.surface, excl: r.betfair_market_id });
        r.surface_p1_wins  = p1surf.wins;  r.surface_p1_total = p1surf.total;
        r.surface_p2_wins  = p2surf.wins;  r.surface_p2_total = p2surf.total;

        // Pick best available data for each player: surface-specific if ≥1, else overall.
        // r.surface_wr_source is 'surface' | 'overall' | 'mixed' to surface in the tooltip.
        const pickPlayer = (surf, name) => {
          if (surf.total >= 1) return { wins: surf.wins, total: surf.total, src: 'surface' };
          const ovr = _overallWrStmt.get({ name, excl: r.betfair_market_id });
          if (ovr.total >= 1) return { wins: ovr.wins, total: ovr.total, src: 'overall' };
          return null;
        };
        const p1 = pickPlayer(p1surf, r.player_a_name);
        const p2 = pickPlayer(p2surf, r.player_b_name);
        if (p1 && p2) {
          r.surface_wr_diff   = +((p1.wins / p1.total - p2.wins / p2.total) * 100).toFixed(1);
          r.surface_wr_source = (p1.src === p2.src) ? p1.src : 'mixed';
          // Store fallback counts so the dashboard tooltip can be accurate
          r.surface_p1_wins_used  = p1.wins;  r.surface_p1_total_used = p1.total;
          r.surface_p2_wins_used  = p2.wins;  r.surface_p2_total_used = p2.total;
        } else {
          r.surface_wr_diff   = null;
          r.surface_wr_source = null;
        }
      }
      // Volume milestones (already a small JOIN — at most 3 rows per market)
      const miles = _milesStmt.all(r.betfair_market_id);
      r.vol_pre_match = miles.find(m => m.milestone === 'pre_match')?.matched_volume ?? null;
      r.vol_set1_end  = miles.find(m => m.milestone === 'set_1_end')?.matched_volume ?? null;
      r.vol_set2_end  = miles.find(m => m.milestone === 'set_2_end')?.matched_volume ?? null;

      // True match_state — used by the dashboard to pick the right status badge.
      // The previous client-side heuristic ("not in liveMatches" → Finished) misclassified
      // in-progress matches whenever the live cache was stale.
      // Rules in priority order:
      //   1. settled (bet has a settlement_type) — already handled client-side
      //   2. market has ended_at → 'finished'
      //   3. final_sets shows a clear winner (2+ sets won by one player) → 'finished'
      //   4. latest snapshot is within 5 min → 'in_progress'
      //   5. otherwise → 'unknown' (stream may have stalled)
      const latestSnapTs = db.prepare(`
        SELECT ts FROM market_snapshots
        WHERE betfair_market_id = ? ORDER BY ts DESC LIMIT 1
      `).get(r.betfair_market_id)?.ts;
      const ended_at = db.prepare(`SELECT ended_at FROM markets WHERE betfair_market_id = ?`).get(r.betfair_market_id)?.ended_at;
      ...
      if (ended_at) {
        ...
        try {
          ...
          let a = 0, b = 0;
          for (const s of sets) {
            const aw = s.playerA ?? s[0];
            const bw = s.playerB ?? s[1];
            if (aw > bw) a++; else if (bw > aw) b++;
          }
          if (a >= 2 || b >= 2) matchState = 'finished';
        } catch (_) {}
      }
      if (matchState !== 'finished' && latestSnapTs) {
        const ageMs = Date.now() - new Date(latestSnapTs).getTime();
        if (ageMs <= 5 * 60_000) matchState = 'in_progress';
      }
      r.match_state = matchState;
    }
...
function apiDbEntryData(req, res) {
  try {
    const _entryDataQuery = db.prepare(`
      SELECT
        b.bet_id, b.strategy_name, b.sub_strategy, b.player_key, b.player_name, b.side,
        b.requested_odds, b.actual_odds, b.stake, b.liability, b.size_matched,
        b.momentum_at_bet, b.edge_at_bet,
        b.pnl, b.settlement_type,
    ...
    for (const row of rows) {
      ...
      byStrategy[strat].push({
        ...
        pnl:            row.pnl,
        roiPct:         (row.stake && row.pnl != null)
                          ? +((row.pnl / row.stake) * 100).toFixed(2)
                          : null,
        outcome,
        ...
        subStrategy:    row.sub_strategy
                          || (row.strategy_name && row.player_key
                                ? (/_P[12]$/.test(row.strategy_name)
                                    ? row.strategy_name
                                    : `${row.strategy_name}-${row.player_key === 'A' ? 'P1' : 'P2'}`)
                                : null),
        momentumAtBet:  row.momentum_at_bet,
  ...
  try {
    const rows = db.prepare(`
      ...
      WHERE m.ended_at IS NOT NULL
        AND COALESCE(pm_end.matched_volume, pm_s1.matched_volume, pm_pre.matched_volume, 0) >= 100000
